...
with left:
    ...
    txn_id   = st.text_input("Transaction ID", "TXN-48219")
    ...
    with c2:
        distance     = st.slider("Distance from home (km)", 0, 5000, 2)
...
**Distance from home**  
&nbsp;&nbsp;<10 km → +0.0 &nbsp;|&nbsp; 500 km → +0.45 &nbsp;|&nbsp; 2000+ → +1.20

**Flags**  
&nbsp;&nbsp;International → +0.40  
&nbsp;&nbsp;Card-not-present online → +0.55  
...
if run:
    pred    = predict_transaction(
        ...
        distance_from_home=distance, hour_of_day=hour_of_day,
    ...
    with col_s:
        ...
        st.markdown(
            srow("Transaction ID",  f"<code>{txn_id}</code>") +
